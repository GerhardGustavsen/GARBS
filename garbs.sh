...
# fork of Luke's Auto Rice Boostrapping Script (GARBS)
# original LARBS by Luke Smith <luke@lukesmith.xyz>
# License: GNU GPLv3

### OPTIONS AND VARIABLES ###

dotfilesrepo="https://github.com/GerhardGustavsen/dotfiles.git"
progsfile="https://raw.githubusercontent.com/GerhardGustavsen/GARBS/blob/master/progs/progs.csv"
aurhelper="yay"
repobranch="master"
export TERM=ansi
...
welcomemsg() {
	whiptail --title "Welcome!" \
		--msgbox "Welcome to Gerhard's Auto-Rice Bootstrapping Script!\\n\\nThis script will automatically install a fully-featured Linux desktop, which I use as my main machine.\\n\\n-Gerhard" 10 60
...
usercheck() {
	! { id -u "$name" >/dev/null 2>&1; } ||
		whiptail --title "WARNING" --yes-button "CONTINUE" \
			--no-button "No wait..." \
			--yesno "The user \`$name\` already exists on this system. GARBS can install for a user already existing, but it will OVERWRITE any conflicting settings/dotfiles on the user account.\\n\\nGARBS will NOT overwrite your user files, documents, videos, etc., so don't worry about that, but only click <CONTINUE> if you don't mind your settings being overwritten.\\n\\nNote also that GARBS will change $name's password to the one you just gave." 14 70
}

preinstallmsg() {
	whiptail --title "Start instalations?" --yes-button "Let's go!" \
...
makeuserjs(){
	...
	userjs="$pdir/user.js"
	ln -fs "/home/$name/.config/firefox/garbs.js" "$overrides"
...
trap 'rm -f /etc/sudoers.d/garbs-temp' HUP INT QUIT TERM PWR EXIT
echo "%wheel ALL=(ALL) NOPASSWD: ALL" >/etc/sudoers.d/garbs-temp
...
echo "%wheel ALL=(ALL:ALL) ALL" >/etc/sudoers.d/00-garbs-wheel-can-sudo
echo "%wheel ALL=(ALL:ALL) NOPASSWD: /usr/bin/shutdown,/usr/bin/reboot,/usr/bin/systemctl suspend,/usr/bin/wifi-menu,/usr/bin/mount,/usr/bin/umount,/usr/bin/pacman -Syu,/usr/bin/pacman -Syyu,/usr/bin/pacman -Syyu --noconfirm,/usr/bin/loadkeys,/usr/bin/pacman -Syyuw --noconfirm,/usr/bin/pacman -S -u -y --config /etc/pacman.conf --,/usr/bin/pacman -S -y -u --config /etc/pacman.conf --" >/etc/sudoers.d/01-garbs-cmds-without-password
echo "Defaults editor=/usr/bin/nvim" >/etc/sudoers.d/02-garbs-visudo-editor
